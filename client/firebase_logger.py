...
class FirebaseLogger:
    ...
    def _make_request_with_retry(self, method: str, path: str, data: Dict = None, max_retries: int = 3) -> Optional[Dict]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.debug(f"✅ Firebase request thành công (attempt {attempt + 1})")
                return response.json() if response.content else {}

            except requests.exceptions.RequestException as e:
                if attempt == max_retries - 1:  # Lần cuối cùng
                    logger.error(f"❌ Firebase request thất bại sau {max_retries} lần thử: {e}")
                    return None
                else:
                    # Exponential backoff: 1s, 2s, 4s...
                    wait_time = 2 ** attempt
                    logger.warning(f"⚠️ Firebase request thất bại (attempt {attempt + 1}/{max_retries}), thử lại sau {wait_time}s: {e}")
                    time.sleep(wait_time)

            except Exception as e:
                logger.error(f"❌ Lỗi Firebase không mong đợi: {e}")
                return None
    ...
    def send_log(self, event: str, path: str = None, fingerprint: Dict = None, once: bool = False):
        ...
        if not self.is_configured():
            logger.warning("⚠️ Firebase chưa được cấu hình - cần tạo config.json")
            print(f"⚠️ Firebase chưa được cấu hình")
            print(f"📁 Cần tạo file: {CONFIG_FILE}")
            print(f"📋 Copy từ: config.json.example")
            return
        ...
        try:
            ...
            if result is not None:
                logger.info(f"✅ Đã gửi log Firebase: {event}")
                print(f"✅ Đã gửi log Firebase: {event}")
            else:
                logger.error(f"❌ Gửi log Firebase thất bại: {event}")
                print(f"❌ Gửi log Firebase thất bại: {event}")

        except Exception as e:
            logger.error(f"❌ Gửi Firebase thất bại: {e}")
            print(f"❌ Gửi Firebase thất bại: {e}")
    ...
    def update_status(self, machine_info: Dict):
        ...
        try:
            ...
            if result is not None:
                logger.debug(f"✅ Đã cập nhật status Firebase")
            else:
                logger.error(f"❌ Cập nhật status Firebase thất bại")

        except Exception as e:
            logger.error(f"❌ Cập nhật Firebase status thất bại: {e}")
            ...
            def check_commands():
                while True:
                    try:
                        ...
                        logger.error(f"❌ Lỗi check commands: {e}")

                    time.sleep(30)  # Check every 30 seconds

            thread = threading.Thread(target=check_commands, daemon=True)
            thread.start()
            logger.info("✅ Đã bắt đầu lắng nghe commands Firebase")

        except Exception as e:
            logger.error(f"❌ Lỗi setup commands listener: {e}")
